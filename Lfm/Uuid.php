...
class Lfm_Uuid {
	...
	public function set($uuid) {
		if(is_string($uuid) && strlen($uuid) === 16) {
			$this->binary = $uuid;
		} elseif(is_string($uuid) && strlen($uuid) === 32 &&
				 preg_match('/^[[:xdigit:]]{32}$/', $uuid))
		{
			$this->binary = '';
			for($i = 0; $i < 32; $i += 2) {
				$this->binary .= chr(hexdec(substr($uuid, $i, 2)));
				 ...
				 (strlen($uuid) === strlen(self::NIL_UUID) ||
				  strlen($uuid) === strlen(self::NIL_UUID) + 2) &&
				 preg_match('/^\{?([[:xdigit:]]{8})-([[:xdigit:]]{4})-([[:xdigit:]]{4})-([[:xdigit:]]{4})-([[:xdigit:]]{12})\}?$/', $uuid, $match))
		{
			...
			throw new Lfm_Uuid_Exception('Invalid uuid format');
		}

		switch($this->getVersion()) {
		case self::VERSION_NIL:
			if($this->binary !== str_repeat(chr(0x00), 16)) {
				throw new Lfm_Uuid_Exception('Uuid verion is zero. But uuid is not `nil uuid`');
			}
			break;
		...
		default:
			throw new Lfm_Uuid_Exception('Invalid uuid version');
		}
		return $this;
	}
	...
	protected function generateHashed($function, $namespace, $id) {
		...
		if(!$namespace instanceof self) {
			throw new Lfm_Uuid_Exception('Invalid namespace');
		}
